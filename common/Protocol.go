...
type JobExecuteInfo struct {
	...
	CancleCtx  context.Context    //用于取消任务的
	CancleFunc context.CancelFunc //用于取消的函数
}
...
type JobLog struct {
	...
	ScheduleTime int64  `json:"scheduleTime" bson:"scheduleTime"` //实际调用时间
	StartTime    int64  `json:"startTime" bson:"startTime"`       //执行任务开始时间
...
//应答方法
func BulidResponse(errno int, msg string, data interface{}) (resp []byte, err error) {
	// 定义一个response
	var (
		response Response
	)
