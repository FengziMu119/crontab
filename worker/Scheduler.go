...
func (scheduler *Scheduler) handleJobEvent(jobEvent *common.JobEvent) {
	...
	case common.JOB_EVENT_KILL:
		// 取消Command执行
		if jobExecuteInfo, jobExcuting = scheduler.jobExecutingTable[jobEvent.Job.Name]; jobExcuting {
			jobExecuteInfo.CancleFunc() // 杀死子进程 任务退出
		}
	}
}
...
func (scheduler *Scheduler) handleJobResult(result *common.JobExecuteResult) {
	...
	//生成执行日志
	if result.Err != common.ERR_LOCK_ALREDY_REQUIRED {
		jobLog = &common.JobLog{
